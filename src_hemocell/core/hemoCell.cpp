...
void HemoCell::setAdhesionTimeScaleSeperation(unsigned int separation){
  hlog << "(HemoCell) (Adhesion Timescale Seperation) Setting seperation to " << separation << " timesteps"<<endl;
  cellfields->adhesionTimescale = separation;
}
...
void HemoCell::setAdhesion(T r0, T rc, T epsilon, T D0, T alpha) {
  if (r0 <= 0.0 || rc <= r0 || epsilon <= 0.0 || D0 <= 0.0 || alpha <= 0.0 ||
      param::dx <= 0.0 || param::df <= 0.0) {
    pcerr << "(HemoCell) (Adhesion) Invalid adhesion parameters." << endl;
    exit(1);
  }
  ...
  cellfields->adhesionR0 = r0*micrometerToLbm;
  cellfields->adhesionCutoff = rc*micrometerToLbm;
  cellfields->adhesionSigma = cellfields->adhesionR0/pow(2.0, 1.0/6.0);
  cellfields->adhesionEpsilon = epsilon*jouleToLbm;
  cellfields->adhesionD0 = D0*jouleToLbm;
  cellfields->adhesionAlpha = alpha/micrometerToLbm;
