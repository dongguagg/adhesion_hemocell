...
void HemoCellParticleField::applyCellCellInteraction(bool adhesion) {
  const T cutoff = adhesion ? cellFields->adhesionCutoff : cellFields->repulsionCutoff;
  ...
  for (int x = 0; x < nx; ++x) {
    for (int y = 0; y < ny; ++y) {
      for (int z = 0; z < nz; ++z) {
        ...
        int lIndexY = y;
        int lIndexZ = z;
        const unsigned int lIndex = grid_index(lIndexX,lIndexY,lIndexZ);

        for (int ox = 0; ox <= neighborRange; ++ox) {
          for (int oy = -neighborRange; oy <= neighborRange; ++oy) {
            if (ox == 0 && oy < 0) { continue; }

            for (int oz = -neighborRange; oz <= neighborRange; ++oz) {
              ...
              const unsigned int nIndex = grid_index(xx,yy,zz);
              const bool sameBin = lIndex == nIndex;

              for (unsigned int i = 0; i < particle_grid_size[lIndex]; ++i) {
                const unsigned int jBegin = sameBin ? i+1 : 0;
                for (unsigned int j = jBegin; j < particle_grid_size[nIndex]; ++j) {
                  HemoCellParticle & lParticle = particles[particle_grid[lIndex][i]];
                  HemoCellParticle & nParticle = particles[particle_grid[nIndex][j]];
                  ...
                  if (distanceSquared <= 0.0 || distanceSquared >= cutoffSquared) { continue; }

                  const T distance = std::sqrt(distanceSquared);
                  T forceMagnitude;
                  if (adhesion) {
                    if (distance < cellFields->adhesionR0) {
                      const T sigmaRatioSquared = cellFields->adhesionSigma*cellFields->adhesionSigma/distanceSquared;
                      const T sigmaRatioSixth = sigmaRatioSquared*sigmaRatioSquared*sigmaRatioSquared;
                      forceMagnitude = 24.0*cellFields->adhesionEpsilon/distance*
                                       (2.0*sigmaRatioSixth*sigmaRatioSixth-sigmaRatioSixth);
                    } else {
                      const T morseExponent = std::exp(-cellFields->adhesionAlpha*
                                                       (distance-cellFields->adhesionR0));
                      forceMagnitude = 2.0*cellFields->adhesionAlpha*cellFields->adhesionD0*
                                       (morseExponent*morseExponent-morseExponent);
                    }
                  } else {
                    forceMagnitude = cellFields->repulsionConstant*(cutoff/distance);
                  }

                  const hemo::Array<T,3> interactionForce = forceMagnitude*(dv/distance);
                  lParticle.sv.force_repulsion = lParticle.sv.force_repulsion + interactionForce;
                  nParticle.sv.force_repulsion = nParticle.sv.force_repulsion - interactionForce;
                }
              }
            }
          }
