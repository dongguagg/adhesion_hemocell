...
int main(int argc, char *argv[]) {
  ...
  if (global::mpi().getSize() != 2) {
    hlog << "(PipeFlow) The 2x1x1 decomposition requires exactly 2 MPI ranks."
         << endl;
    return EXIT_FAILURE;
  }
  ...
  // Explicit axial decomposition: one block per rank, no split in y or z.
  const SparseBlockStructure3D blockStructure =
      createRegularDistribution3D(flagMatrix->getBoundingBox(), 2, 1, 1);
  const MultiBlockManagement3D management(
      blockStructure, new OneToOneThreadAttribution,
      (*cfg)["domain"]["fluidEnvelope"].read<int>());
  // Keep the STL fluid/solid mask aligned with the simulation lattice.
  flagMatrix.reset(flagMatrix->clone(management));
  ...
  hlog << "(PipeFlow) Explicit MPI decomposition: 2x1x1 (x,y,z), "
          "one block per rank." << endl;
  ...
  // Use the same LJ/Morse cell-cell adhesion parameters as bifurcation.
  hemocell.setAdhesion((*cfg)["cellCellAdhesion"]["r0"].read<T>(),
  ...
  // Both interactions share force_repulsion: clear/recompute them together
  // whenever particle velocities are updated (5 steps in the original case).
  const unsigned int interactionEvery =
      (*cfg)["ibm"]["stepParticleEvery"].read<unsigned int>();
  if (interactionEvery == 0) {
    hlog << "(PipeFlow) stepParticleEvery must be positive." << endl;
    return EXIT_FAILURE;
  }
  ...
  // Official wall repulsion: kRep is already in LBM force units; the API
  // converts only RepCutoff from micrometers to lattice units.
  const T wallRepulsionLbm = (*cfg)["boundaryRepulsion"]["kRep"].read<T>();
  const T wallCutoffUm = (*cfg)["boundaryRepulsion"]["RepCutoff"].read<T>();
  hemocell.enableBoundaryParticles(wallRepulsionLbm, wallCutoffUm,
                                   interactionEvery);
  hlog << "(PipeFlow) Wall repulsion coefficient: " << wallRepulsionLbm
       << " LBM = " << wallRepulsionLbm * param::df << " N; cutoff: "
       << wallCutoffUm << " um. Interactions update every "
       << interactionEvery << " steps." << endl;
